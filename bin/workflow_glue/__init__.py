...
def get_components(allowed_components=None):
    ...
    for fname in globs:
        ...
        try:
            ...
            # if imports cannot be satisfied, refuse to add the component
            # rather than exploding
            missing = getattr(e, "name", str(e))
            logger.warn(f"Could not load {name} due to missing module {missing}")
            continue
...
def cli():
    ...
    # Import only the requested command when possible; fall back to all for --help.
    if len(sys.argv) > 1 and not sys.argv[1].startswith('-'):
        cmd = sys.argv[1]
        components = get_components(allowed_components=[cmd])
        if cmd not in components:
            logger.error(
                "Command %r not available (unknown command or missing dependencies).",
                cmd)
            sys.exit(1)
    else:
