...
def argparser():
    ...
    parser.add_argument(
        "--min_read_coverage", type=float, default=0.4,
...
def main(args):
    ...
    df_gffcompare_tmap = pd.read_csv(
        args.gffcompare_tmap, index_col=None, sep='\t', comment='#')
    ...
    for df in parse_bam(args.transcriptome_bam, args.chunksize):
        ...
        # Inner join: reads must have a transcript assignment and a genomic mapq score
