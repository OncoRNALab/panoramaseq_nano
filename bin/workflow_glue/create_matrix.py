...
def cluster_dataframe(df, ref_interval, umi_length):
    ...
    valid_coords = (
        df_no_gene['start'].notna() & df_no_gene['end'].notna()
        & (df_no_gene['chr'] != '-'))
    df_no_gene_valid = df_no_gene.loc[valid_coords]
    if len(df_no_gene_valid) > 0:
        # Create a temporary gene name based on chr and location.
        regions = df_no_gene_valid.apply(
...
def main(args):
    ...
    # UB is only required in barcode_tags when skipping clustering (pre-corrected UMIs).
    input_has_ub_col = args.skip_umi_clustering
    ...
    for chunk_num, df_tags in enumerate(
        ...
        df_tags[['gene', 'transcript']] = df_tags[['gene', 'transcript']].fillna('-')
        df_tags['start'] = pd.to_numeric(df_tags['start'], errors='coerce')
        df_tags['end'] = pd.to_numeric(df_tags['end'], errors='coerce')
        ...
        if args.stats:
            logger.info(f"Writing JSON stats to {args.stats}")
